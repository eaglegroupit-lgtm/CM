...
export default async function PurchaseVoucherPage() {
  const supabase = await createClient();
  const [{ data: parties }, { data: stockItems }, { data: units }, { data: godowns }, { data: company }, systemLedgers] =
    await Promise.all([
      supabase.from("ledgers").select("*").in("party_type", ["creditor", "both"]).eq("is_active", true).order("name"),
  ...
  return (
    <Tabs defaultValue="new">
      ...
      <TabsContent value="recent">
        <Card>
          <CardHeader>
            <CardTitle>Recent Purchase Vouchers</CardTitle>
          </CardHeader>
          <CardContent>
            <RecentVouchers typeCode="PURC" />
          </CardContent>
        </Card>
      </TabsContent>
    </Tabs>
